    ...
    <div className="bg-zinc-100 w-full min-h-screen flex flex-col items-center">
      ...
      <div className="w-full max-w-4xl flex flex-row bg-[#E4EBF1] mt-6 rounded-2xl overflow-hidden mx-4">
        <button
          ...
          className={`flex-1 py-1.5 m-1  rounded-xl font-medium text-sm flex items-center justify-center gap-3 transition-all ${trailType !== 'Andamento' ? 'hover:bg-zinc-100' : 'hover:bg-[#2563EB]'} ${trailType === 'Andamento' ? 'bg-[#2563EB] text-white font' : 'bg-[#E4EBF1]'}`}
        >
          <TrendingUp size={16}></TrendingUp> Em Andamento
        </button>
        <button
          onClick={() => setTrailType('Personalizadas')}
          className={`flex-1 py-1.5 m-1 rounded-xl font-medium text-sm flex items-center justify-center gap-3 transition-all ${trailType !== 'Personalizadas' ? 'hover:bg-zinc-100' : 'hover:bg-[#2563EB]'} ${trailType === 'Personalizadas' ? 'bg-[#2563EB] text-white' : 'bg-[#E4EBF1]'}`}
        >
          <Sparkles size={16}></Sparkles> Personalizadas
        </button>
        <button
          onClick={() => setTrailType('Concluidas')}
          className={`flex-1 py-1.5 m-1 rounded-xl font-medium text-sm flex items-center justify-center gap-3 transition-all ${trailType !== 'Concluidas' ? 'hover:bg-zinc-100' : 'hover:bg-[#2563EB]'} ${trailType === 'Concluidas' ? 'bg-[#2563EB] text-white' : 'bg-[#E4EBF1]'}`}
        >
